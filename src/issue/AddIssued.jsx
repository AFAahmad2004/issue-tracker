...
const AddIssue = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const formattedData = {
      data: {
        ...
        imageUrl: "string", 
        counter: 0,
        issueStatus: data.status.charAt(0).toUpperCase() + data.status.slice(1),  
        username: "test",  
        locale: "string" 
      }
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        setServerMessage(
          result.message || `حدث خطأ أثناء إضافة المشكلة: ${result.error?.message || "غير محدد"}`
        );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg mx-auto p-6 bg-gradient-to-r from-blue-800 to-blue-900 shadow-xl rounded-xl border border-gray-300">
    <h2 className="text-4xl font-extrabold mb-8 text-center text-white">إضافة مشكلة جديدة</h2>
  
    {/* حقل اسم المشكلة */}
    <div className="mb-6">
      <label htmlFor="issueName" className="block mb-2 text-lg font-medium text-white">
        اسم المشكلة
      </label>
      <input
        type="text"
      ...
      />
      {errors.issueName && <p className="text-red-500 text-sm mt-2">{errors.issueName.message}</p>}
    </div>
  
    {/* حقل وصف المشكلة */}
    <div className="mb-6">
      <label htmlFor="issueDescription" className="block mb-2 text-lg font-medium text-white">
        وصف المشكلة
      </label>
      <textarea
        id="issueDescription"
      ...
      ></textarea>
      {errors.issueDescription && <p className="text-red-500 text-sm mt-2">{errors.issueDescription.message}</p>}
    </div>
  
    {/* حقل حالة المشكلة */}
    <div className="mb-6">
      <label htmlFor="status" className="block mb-2 text-lg font-medium text-white">
        حالة المشكلة
      </label>
      <select
        id="status"
    ...
    {/* رسالة من الخادم */}
    {serverMessage && <p className="mt-4 text-center text-lg font-medium text-white">{serverMessage}</p>}
  </form>
